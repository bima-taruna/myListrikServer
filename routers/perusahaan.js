...
router.get(`/:id`, async (req,res)=>{
    ...
    if(!perusahaan) {
        res.status(500).json({message: 'perusahaan dengan id yang diberikan tidak ditemukan'});
    }
...
router.put(`/:id`,authJwt, async (req,res)=>{
    ...
    const perusahaan = await Perusahaan.findByIdAndUpdate(req.params.id,{
        name : req.body.name,
        city : req.body.city
    }, {new:true}).populate('city');
    if(!perusahaan) {
...
router.delete('/:id',authJwt, (req,res)=>{
    ...
    Perusahaan.findByIdAndRemove(req.params.id).then(perusahaan =>{
        if(perusahaan){
           return res.status(200).json({success:true,message:'data perusahaan berhasil dihapus!'});
        } else {
