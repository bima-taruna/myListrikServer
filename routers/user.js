...
//GETBYROLE = Teknisi
router.get(`/teknisi/all`, authJwt, async (req, res) => {
  if (req.auth.role !== "admin") {
    return res.status(401).json({
      message: "anda tidak memiliki izin untuk mengakses laman ini",
      success: false,
...
router.get(`/:id`, authJwt, async (req, res) => {
  ...
  if (!user) {
    res
      .status(500)
      .json({ message: "user dengan id yang diberikan tidak ditemukan" });
  }
...
router.post(`/`, authJwt, uploadOptions.single("avatar"), async (req, res) => {
  if (req.auth.role !== "admin") {
    ...
    if (userExist) {
      ...
      if (!city)
        return res.status(400).send("data kota belum dipilih atau tidak ada");
      ...
      if (req.body.role !== "user") {
        ...
        if (!perusahaan)
          return res
            .status(400)
            .send("data perusahaan belum dipilih atau tidak ada");
      }
